...
export function RecordsScreen({records}:{records:RecordItem[]}){
  ...
  return <main className="screen records-screen">
    ...
    <section className="week-card">
      <div className="small-ring" style={{'--progress':rate*3.6+'deg'} as CSSProperties}><strong>{rate}%</strong></div>
      <div><span>18種類チャレンジ</span><strong>{completedKinds.size} / {exerciseIds.length} 種類</strong></div>
    </section>
    <section className="summary">
      <div><Trophy/><span>達成した種類</span><strong>{completedKinds.size}種類</strong></div>
      <div><Flame/><span>今週の消費カロリー</span><strong>{kcal.toFixed(1)}kcal</strong></div>
    </section>
    <h2>今週の活動サマリー</h2>
    ...
    <div className="record-heading">
      <h2>運動の記録</h2>
      {records.length>weekRecords.length&&<button className="text-button compact" onClick={()=>setShowAll(x=>!x)}>{showAll?'今週のみ':'すべて見る'}<ChevronRight/></button>}
    </div>
    <section className="record-list">{records.length===0?<p>運動を行うと、ここに記録されます。</p>:visibleRecords.length===0?<p>今週の記録はまだありません。</p>:visibleRecords.map(record=>{
      ...
      return <article key={record.id}>
        <div className={'record-icon '+(ok?exercise.intensity:'incomplete')}>{ok?<Check/>:<Square/>}</div>
        <div><strong>{exercise.name}</strong><span>{record.minutes}分・{record.kcal}kcal</span></div>
        <div className="record-result"><b className={ok?'done':'not-done'}>{ok?'達成':'未達成'}</b><time>{new Intl.DateTimeFormat('ja-JP',{month:'numeric',day:'numeric'}).format(new Date(record.completedAt))}</time></div>
      </article>;
    })}</section>
  </main>;
}
